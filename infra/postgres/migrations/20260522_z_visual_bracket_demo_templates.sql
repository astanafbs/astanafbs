  ...
  LOOP
    ...
    FOR round_number IN REVERSE total_rounds..1 LOOP
      ...
      FOR bracket_position IN 1..match_count LOOP
        ...
        VALUES (
          uuid_generate_v5(uuid_ns_url(), 'visual-bracket-' || tournament_record.slug || '-r' || round_number || '-m' || bracket_position),
          ...
          CASE
            WHEN round_number < total_rounds
              THEN uuid_generate_v5(uuid_ns_url(), 'visual-bracket-' || tournament_record.slug || '-r' || (round_number + 1) || '-m' || ceil(bracket_position / 2.0)::integer)
            ELSE NULL
          END,
          CASE
            WHEN round_number < total_rounds
